...
const generateAccessandrefreshToken = async(userId) => 
{
    try {
        ...
        throw new ApiError("Something went wrong while generating acces and refresh tokens")
    }
}
...
const changeCurrentPassword = asyncHandler(async(req,res) => {
    ...
    const user = await User.findById(req.user?._id) //user is logged in so the id is taken from there
...
const getUserChannelProfile = asyncHandler(async(req,res) => {
    ...
    .status(200)
    .json(
        new ApiResponse(200,"Channel fetches succesfully")
    )
})
